        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-[#4A5568] mb-1">
              Fecha de la actividad (Mes de Septiembre) *
            </label>
            <input 
              type="date" required 
              min="2026-09-01"
              ...
              className={`input-brand w-full px-4 py-3 rounded-lg ${
                scheduledDate && scheduledDate.split('-')[1] !== '09' 
                  ? 'border-red-500 bg-red-50/30 ring-2 ring-red-200' 
                  : ''
              }`}
            />
            {scheduledDate && scheduledDate.split('-')[1] !== '09' && (
              <div className="mt-1.5 p-2.5 rounded-lg bg-red-50 border border-red-200 flex items-start gap-2 text-xs text-red-700 font-medium animate-fade-in" role="alert">
                <svg className="w-4 h-4 text-red-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                <div>
                  <span className="font-bold block">Fecha restringida a Septiembre</span>
                  La campaña &quot;A Limpiar el Mundo&quot; se realiza exclusivamente durante el mes de septiembre. Por favor selecciona una fecha entre el 1 y el 30 de septiembre de 2026.
                </div>
              </div>
            )}
            {scheduledDate && scheduledDate.split('-')[1] === '09' && (
              <div className="mt-1 text-[11px] text-emerald-700 font-semibold flex items-center gap-1">
                ...
                Fecha válida de campaña (Septiembre 2026)
              </div>
            )}
          </div>
